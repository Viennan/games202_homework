...
vec3 EvalDiffuse(vec3 wi, vec3 wo, vec2 uv) {
  vec3 k = GetGBufferDiffuse(uv);
  vec3 normal = GetGBufferNormalWorld(uv);
  float weight = dot(wi, normal);
  if (weight < 0.0) {
    return vec3(0.0);
  }
  return k * weight;
...
vec3 EvalDirectionalLightWithOutput(vec2 uv, vec3 wo) {
...
vec3 EvalDirectionalLight(vec2 uv) {
  ...
  return EvalDirectionalLightWithOutput(uv, wo);
}
...
bool RayMarch(vec3 ori, vec3 dir, out vec3 hitPos) {
  ...
  for (int i=0;i<MAX_STEPS;i++) {
    ...
    if (atFront(curr + st * dir)) {
      curr += st * dir;
    }
    else {
      hit = true;
      if (st <= EPS) {
        if (hasInter(curr, 2.0 * st * dir, hitPos)) {
          return true;
        }
      }
    }
...
vec3 EvalInDirectionalLight(vec2 uv) {
  vec3 posW = GetGBufferPosWorld(uv);
  vec3 wo = normalize(uCameraPos - posW);
  vec3 n = GetGBufferNormalWorld(uv);
  float weight = dot(wo, n);
  if (weight < 0.0) {
    return vec3(0.0);
  }
  // vec3 wi = normalize(2.0 * weight * n - wo);
  // vec3 hitPos;
  // if (RayMarch(posW, wi, hitPos)) {
  //   return EvalDirectionalLightWithOutput(GetScreenCoordinate(hitPos), -wi) * EvalDiffuse(wi, wo, uv);
  // }
  // return vec3(0.0);
  float seed = InitRand(uv), pdf;
  vec3 L = vec3(0.0);
  vec3 b1, b2;
  LocalBasis(n, b1, b2);
  ...
  for (int i=0;i<SAMPLE_NUM;i++) {
    ...
    if (RayMarch(posW, dir, hitPos)) {
      L += EvalDirectionalLightWithOutput(GetScreenCoordinate(hitPos), -dir) * EvalDiffuse(dir, wo, uv) / pdf;
    }
  }
