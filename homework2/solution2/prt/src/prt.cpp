            ...
            for (int y = 0; y < height; y++)
            {
                for (int x = 0; x < width; x++)
                {
                    ...
                    for (int l=0;l<=SHOrder;++l) {
                        int m_bound = l + 1;
                        for (int m=-m_bound+1;m<m_bound;++m) {
                            SHCoeffiecents[sh::GetIndex(l, m)] += float(sh::EvalSH(l, m, dir_d)) * Le * CalcArea(x, y, width, height);
                        }
                    }
                }
            }
...
public:
    ...
    virtual void preprocess(const Scene *scene) override
    {
        ...
        if (m_Type == Type::Interreflection)
        {
            ...
            Eigen::MatrixXf preIterTransSHCoeffs = m_TransportSHCoeffs;
            for (int b=0;b<m_Bounce;++b) {
                Eigen::MatrixXf iterTransSHCoeffs;
                iterTransSHCoeffs.resize(SHCoeffLength, mesh->getVertexCount());
                for (int i = 0; i < mesh->getVertexCount(); i++)
                {
                    const Point3f &v = mesh->getVertexPositions().col(i);
                    const Normal3f &n = mesh->getVertexNormals().col(i);
                    ...
                    auto shFunc = [&](double phi, double theta) -> double {
                        ...
                        if (scene->rayIntersect(Ray3f(v, wi), its)) {
                            ...
                            for (int t=0;t<3;++t) {
                                for (int o=0;o<SHCoeffLength;++o) {
                                    coeffs[o] += its.bary[t] * preIterTransSHCoeffs.col(its.tri_index[t]).coeffRef(o);
                                }
                            }

                            auto reflectDir = v - its.p;
                            double indir = sh::EvalSHSum(SHOrder, coeffs, Eigen::Vector3d(reflectDir.x(), reflectDir.y(), reflectDir.z()).normalized());
                            return std::max(indir * H, 0.0);                  
                        }
                        return 0.0;
                    };
                    ...
                    for (int j = 0; j < shCoeff->size(); j++)
                    {
                        iterTransSHCoeffs.col(i).coeffRef(j) = (*shCoeff)[j];
                        m_TransportSHCoeffs.col(i).coeffRef(j) += (*shCoeff)[j];
                    }
                }
                preIterTransSHCoeffs = iterTransSHCoeffs;
            }
        }
